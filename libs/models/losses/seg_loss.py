...
class EnhancedCLRNetSegLoss(torch.nn.Module):
    ...
    def compute_edge_weights(self, targets):
        ...
        edge_kernel = edge_kernel.view(1, 1, 3, 3).repeat(self.num_classes, 1, 1, 1)
        ...
        edge_weights = torch.exp(self.edge_weight * edges.abs().sum(dim=1, keepdim=True))
        return edge_weights
    ...
    def forward(self, preds, targets):
        ...
        # 计算加权交叉熵损失
        loss = F.nll_loss(log_probs, targets.long(), weight=self.weights, ignore_index=self.ignore_label, reduction='none')
