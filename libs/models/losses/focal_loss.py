...
class EnhancedFocalLoss(nn.Module):
    ...
    def compute_edge_weights(self, target):
    ...
    def compute_curve_weights(self, target):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        input_soft: torch.Tensor = F.softmax(input, dim=1) + self.eps
        ...
        # Compute edge and curve weights
        edge_weights = self.compute_edge_weights(target)
        curve_weights = self.compute_curve_weights(target)
        ...
        # Adaptive alpha
        alpha = self.alpha * (1 - input_soft) + (1 - self.alpha) * input_soft

        focal = -alpha * weight * torch.log(input_soft)
        loss_tmp = torch.sum(target_one_hot * focal, dim=1)

        # Apply combined weights
        loss_tmp = loss_tmp * combined_weights
